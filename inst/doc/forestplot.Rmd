...
[Forest plots](http://en.wikipedia.org/wiki/Forest_plot) date back to [1970s](http://www.ncbi.nlm.nih.gov/pmc/articles/PMC1120528/) and are most frequently seen in [meta-analysis](http://en.wikipedia.org/wiki/Meta-analysis), but are in no way restricted to these.  The `forestplot` function of this package is a development of the ['rmeta'](http://cran.r-project.org/web/packages/rmeta/index.html)-package's `forestplot` function. 

Apart from generating a basic forest plot merging text with confidence lines and summary indicators, the `forestplot` has a few additional capabilities:

* **Text:**
    + Ability to use a table of text, i.e. the text can consist of several columns if needed&Dagger;
    + Allows expressions within your text that permit mathematical symbols, e.g. `expression(beta)`
    + Set gpar arguments (`fontfamily`, `fontface`, `cex`, etc)  for both summary and regular rows. This can be specified down to the each cell.
* **Confidence intervals:**
    ...
    + Estimate marker choice between boxes, diamonds, points
    + Make your own custom confidence interval drawing functions
* **Legends:**
    + Have legend on top or to the left of the plot
...
We can also choose what lines we want:

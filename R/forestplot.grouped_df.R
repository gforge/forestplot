...
forestplot.grouped_df <- function(x, labeltext, mean, lower, upper, legend, is.summary, boxsize, ...) {
  ...
  get_expr_name <- function(value) {
    tryCatch(rlang::as_name(value), error = function(e) NULL)
  }

  mean_name <- get_expr_name(mean)
  lower_name <- get_expr_name(lower)
  upper_name <- get_expr_name(upper)
  ...
  core_data <- dplyr::bind_cols(
    fp_label_core_values,
  ...
  if (!is.null(is.summary)) {
    ...
    if (length(fixed_is.summary) != nrow(fixed_data)) {
      stop(
        "Expected is.summary to have the length ", fixed_data |> dplyr::filter(.fp_groups == .fp_groups[1]) |> nrow(),

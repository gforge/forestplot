...
fpTxtGp <- function(label,
  ...
  ret$summary <-
    lMerge(ret$label,
           list(fontface = "bold",
                      cex = ret$label$cex*1.1))

  if (!missing(summary)){
    if (class(summary) != "gpar")
      stop("You can only provide arguments from gpar() to the function")
    ret$summary <- lMerge(ret$summary,
                          summary)
  }

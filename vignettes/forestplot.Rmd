...
```{r}
data(HRQoL)
dfHRQoL <- sapply(names(HRQoL), function(n) {
  HRQoL[[n]] %>%
    as_tibble(rownames = "labeltext") %>%
    mutate(group = n)
}, simplify = FALSE) %>%
  bind_rows() %>% 
  rename(mean = coef)
...
As of version *2.0* the forestplot package is compatible with standard `dplyr` syntax. As seen above many of the examples already uses this syntax. Below is a simple illustration of how it works. If you provide a `data.frame` it will assume that the names are `mean`, `lower`, `upper` and `labeltext` unless you specify otherwise.
